...
test_utf8_valid(void **state)
{
    ...
    assert_true(sb_utf8_validate((uint8_t*) c, strlen(c)));
    const uint8_t d[3] = {0xe2, 0x82, 0xac};  // euro sign
    assert_true(sb_utf8_validate(d, 3));
    const uint8_t e[3] = {0xef, 0xbb, 0xbf};  // utf-8 bom
    assert_true(sb_utf8_validate(e, 3));
}
...
test_utf8_invalid(void **state)
{
    ...
    assert_false(sb_utf8_validate(c, 4));
    const uint8_t d[8] = {0xff, 0xfe, 0x00, 0x00, 0xac, 0x20, 0x00, 0x00};  // utf-32
    assert_false(sb_utf8_validate(d, 8));
}
...
test_utf8_valid_str(void **state)
{
    ...
    sb_string_append(s,
        ...
        "posts</a>");
    assert_true(sb_utf8_validate_str(s));
...
test_utf8_invalid_str(void **state)
{
    ...
    assert_false(sb_utf8_validate_str(s));
    sb_string_free(s, true);
    s = sb_string_new();
...
static void
test_utf8_bom_length(void **state)
{
    const uint8_t c[4] = {0xef, 0xbb, 0xbf, 0};
    assert_int_equal(sb_utf8_bom_length(c, 2), 0);
    assert_int_equal(sb_utf8_bom_length(c, 3), 3);
    assert_string_equal(c + 3, "");
    const uint8_t d[8] = {0xef, 0xbb, 0xbf, 'b', 'o', 'l', 'a', 0};
    assert_int_equal(sb_utf8_bom_length(d, 7), 3);
    assert_string_equal(d + 3, "bola");
    const uint8_t e[5] = "bola";
    assert_int_equal(sb_utf8_bom_length(e, 4), 0);
}
...
main(void)
{
    const UnitTest tests[] = {
        ...
        unit_test(test_utf8_bom_length),
    };
